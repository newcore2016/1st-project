...
class GameBoardVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let refView = UIView()
        refView.frame = CGRect(x: 10, y: 20, width: UIScreen.main.bounds.width-20 , height: (UIScreen.main.bounds.height - 30 ) / 2)
        let imageView = UIImageView()
        imageView.frame = CGRect(x: refView.frame.width/2, y: 0 , width: refView.frame.width/2, height: refView.frame.height/2)
    ...
    // create game board
    func makeGameBoard(){
        boardGame.frame = CGRect(x: 10, y: UIScreen.main.bounds.height/2 + 10, width: UIScreen.main.bounds.width-20 , height: (UIScreen.main.bounds.height - 30 )/2)
        let image = UIImage(named: photoName!)
        for i in 1...colNo! {
            for j in 1...rowNo! {
